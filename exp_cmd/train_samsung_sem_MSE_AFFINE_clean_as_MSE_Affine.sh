...
DATE="221020"
echo $DATE"=== MSE_AFFINE ===SET"${SET_NUM}
CUDA_VISIBLE_DEVICES=$GPU_NUM python main.py --date $DATE --seed 0 --noise-type 'Poisson-Gaussian' --train-with-MSEAffine --loss-function 'MSE_Affine' --model-type 'FBI_Net' --data-type $DATA_TYPE --data-name $DATA_NAME --set-num $SET_NUM --alpha $ALPHA --beta $BETA --batch-size $BATCH_SIZE --lr 0.001 --num-layers 17 --num-filters 64 --crop-size 256 --pge-weight-dir $pge_net_weight_file
