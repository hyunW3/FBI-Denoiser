...
if __name__ == '__main__':
    ...
    for batch_size, (noisy_input, noisy_target, clean) in enumerate(train_loader):
        ...
        print(clean.shape)
        break
## TODO apply dataloder 230407
class N2N_setting(torch.utils.data.Dataset):
    """Class for the denoising dataset for both train and test, with 
    file structure:
        data_root/type/noise_level/fov/capture.png
        type:           12
    ...
    Args:
        ...
        loader (callable, optional): image loader
    """
    def __init__(self, root, train, x_noise_levels,y_noise_levels, types=None, test_fov=19,
        captures=50, transform=None, target_transform=None, loader=pil_loader):
        super().__init__()
        all_noise_levels = [1, 2, 4, 8, 16]
        all_types = ['TwoPhoton_BPAE_R', 'TwoPhoton_BPAE_G', 'TwoPhoton_BPAE_B',
...
BPAE_types = ['TwoPhoton_BPAE_R', 'TwoPhoton_BPAE_G', 'TwoPhoton_BPAE_B',
            ...
            'WideField_BPAE_R', 'WideField_BPAE_G', 'WideField_BPAE_B']
# only use MICE
MICE_types = ['TwoPhoton_MICE', 'Confocal_MICE']
# only use FISH
FISH_types = ['Confocal_FISH']
type_dict = {"ALL": all_types, "BPAE": BPAE_types, "MICE": MICE_types, "FISH": FISH_types}

def get_FMD_dataloader(data_root='/mnt/ssd/hyun/datasets/FMD_dataset', train = True, 
                       img_type='BPAE',noise_levels_train = [1,2,4,8,16],
                       default_transform='center_crop', imsize=256, 
                       batch_size=64,test_group = 19,args=None):
    """
        FMD dataloader with (256, 256) sized images.
    """
    # Compose transforms
    
    if default_transform == 'four_crop':
    ...
    elif default_transform == 'center_crop':
        # default transform
        transform = None
    
    img_type = type_dict[img_type]

    img_size = (imsize,imsize,1)
    

    train_loader,test_loader = load_denoising_onlyNoisyImage(data_root, batch_size=32, 
                                                noise_levels=noise_levels_train, types=img_type,
            loader=pil_loader, transform=transform, patch_size=imsize)
