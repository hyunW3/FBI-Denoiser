...
__all__ = ['fluore_to_tensor', 'DenoisingFolder', 'DenoisingFolderN2N', 
           ...
           'OnlyReturnNoisyImage','load_denoising_onlyNoisyImage',
           'load_denoising_rn2n']
...
class OnlyReturnNoisyImage(torch.utils.data.Dataset):
...
def load_denoising_onlyNoisyImage(root, train, batch_size, noise_levels, types=None, captures=2,
    ...
    dataset = OnlyReturnNoisyImage(root, train, noise_levels, 
...
class ReturnRN2N(torch.utils.data.Dataset):
    """Class for the denoising dataset for both train and test, with 
    file structure:
        data_root/type/noise_level/fov/capture.png
        type:           12
    ...
    Args:
        ...
        train (bool): Training set if True, else Test set
        # noise_levels (seq): e.g. [1, 2, 4] select `raw`, `avg2`, `avg4` folders
        x_noise_level (int) : noise level of x ex) 1,2,4,8,16
        y_noise_level (int) : noise level of y
        types (seq, optional): e.g. ['TwoPhoton_BPAE_B', 'Confocal_MICE`]
        test_fov (int, optional): default 19. 19th fov is test fov
        captures (int): select # images within one folder
        transform (callable, optional): A function/transform that takes in 
        ...
        loader (callable, optional): image loader
    """
    def __init__(self, root, train, x_noise_level,y_noise_level, types=None, test_fov=19,
        captures=50, transform=None, target_transform=None, loader=pil_loader):
        super().__init__()
        all_noise_levels = [1, 2, 4, 8, 16]
        all_types = ['TwoPhoton_BPAE_R', 'TwoPhoton_BPAE_G', 'TwoPhoton_BPAE_B',
            ...
            'Confocal_BPAE_G', 'Confocal_BPAE_B', 'Confocal_FISH',
            'WideField_BPAE_R', 'WideField_BPAE_G', 'WideField_BPAE_B']
        assert all([x_noise_level in all_noise_levels, y_noise_level in all_noise_levels])
        ...
        dataset_info = {'Dataset': 'train' if train else 'test',
                        'x_noise_level': x_noise_level,
                        'y_noise_level': y_noise_level,
                        f'{len(self.types)} Types': self.types,
                        'Fovs': self.fovs,
                        '# samples': len(self.samples['gt'])
                        }
        print(json.dumps(dataset_info, indent=4))
        
    def return_noise_dir(self,subdir : str, noise_level : str):
        if noise_level == 1:
            noise_dir = os.path.join(subdir, 'raw')
        elif noise_level in [2, 4, 8, 16]:
            noise_dir = os.path.join(subdir, f'avg{noise_level}')
        return noise_dir
    def return_noisy_captures(self,noisy_fov_dir : str):
        noisy_captures = []
        # print(os.listdir(noisy_fov_dir))
        for fname in sorted(os.listdir(noisy_fov_dir))[:self.captures]:
            if is_image_file(fname):
                noisy_file = os.path.join(noisy_fov_dir, fname)
                noisy_captures.append(noisy_file)
        return noisy_captures
    ...
    def _gather_files(self):
        ...
        for subdir in subdirs:
            gt_dir = os.path.join(subdir, 'gt')
            x_noise_dir = self.return_noise_dir(subdir, self.x_noise_level)
            y_noise_dir = self.return_noise_dir(subdir, self.y_noise_level)
            
            for i_fov in self.fovs:
                x_noisy_fov_dir = os.path.join(x_noise_dir, f'{i_fov}')
                y_noisy_fov_dir = os.path.join(y_noise_dir, f'{i_fov}')
                clean_file = os.path.join(gt_dir, f'{i_fov}', 'avg50.png')
                
                x_noisy_captures = self.return_noisy_captures(x_noisy_fov_dir)
                y_noisy_captures = self.return_noisy_captures(y_noisy_fov_dir)
                if self.x_noise_level == self.y_noise_level:
                    y_noisy_captures = np.roll(y_noisy_captures, -1)
                # print(len(x_noisy_captures),len(y_noisy_captures))
                for i in range(len(y_noisy_captures)):
                    samples['x'].append(x_noisy_captures[i])
                    samples['y'].append(y_noisy_captures[i])
                    samples['gt'].append(clean_file)
    ...
    def __getitem__(self, index):
        ...
        Args:
            index (int): Index
        Returns:
            tuple: (x_noisy, y_noisy, clean)
        """
            
        x_noisy_file, y_noisy_file = self.samples['x'][index], self.samples['y'][index]
        clean_file = self.samples['gt'][index]
        # print(x_noisy_file,y_noisy_file, clean_file )
        x_noisy = self.loader(x_noisy_file)
        y_noisy = self.loader(y_noisy_file)
        clean = self.loader(clean_file)
        if self.transform is not None:
            x_noisy = self.transform(x_noisy)
            y_noisy = self.transform(y_noisy)
        if self.target_transform is not None:
            clean = self.target_transform(clean)
        # print(x_noisy.shape,y_noisy.shape,clean.shape)
        return x_noisy, y_noisy, clean

    def __len__(self):
        return len(self.samples['y'])
...
all_types = ['TwoPhoton_BPAE_R', 'TwoPhoton_BPAE_G', 'TwoPhoton_BPAE_B',
            ...
            'Confocal_BPAE_G', 'Confocal_BPAE_B', 'Confocal_FISH',
            'WideField_BPAE_R', 'WideField_BPAE_G', 'WideField_BPAE_B']
# only use BPAE images
BPAE_types = ['TwoPhoton_BPAE_R', 'TwoPhoton_BPAE_G', 'TwoPhoton_BPAE_B',
            'Confocal_BPAE_R', 'Confocal_BPAE_G', 'Confocal_BPAE_B',
            'WideField_BPAE_R', 'WideField_BPAE_G', 'WideField_BPAE_B']
# only use MICE
MICE_types = ['TP_MICE', 'CF_MICE'] # ['TwoPhoton_MICE', 'Confocal_MICE']
# only use FISH
FISH_types = ['CF_FISH'] # ['Confocal_FISH']
type_dict = {"ALL": all_types, "BPAE": BPAE_types, "MICE": MICE_types, "FISH": FISH_types}
sym_type_dict = { "TP_MICE": ['TwoPhoton_MICE'], "CF_MICE": ['Confocal_MICE'], "CF_FISH": ['Confocal_FISH']}

def load_denoising_rn2n(data_root='/mnt/ssd/hyun/datasets/FMD_dataset', train = True, 
                        img_type='BPAE',x_noise_level=1, y_noise_level=2,
                       transform_setting='center_crop', patch_size=256, captures = 50,
                       batch_size=64,test_fov = 19,args=None):
    """
        FMD dataloader with (256, 256) sized images.
    """
    # Compose transforms
    print(f'Loading FMD dataset... {img_type} {x_noise_level} {y_noise_level}')
    if type(x_noise_level) == str and x_noise_level[0] == 'F':
        x_noise_level = int(x_noise_level[1:])
    if type(y_noise_level) == str and y_noise_level[0] == 'F':
        y_noise_level = int(y_noise_level[1:])
        
    if transform_setting == 'four_crop':
    ...
    elif transform_setting == 'center_crop':
        # default transform
        transform = transforms.Compose([
            transforms.CenterCrop(patch_size),
            fluore_to_tensor,
        ...
        raise NotImplementedError('Not implemented transform: {}'.format(transform_setting))
    target_transform = transform
    # img_type = type_dict[img_type]
    img_type = sym_type_dict[img_type]
    img_size = (patch_size,patch_size,1)
    

    dataset = ReturnRN2N(data_root, train, 
                                x_noise_level=x_noise_level, y_noise_level=y_noise_level, 
                                types=img_type, captures=captures, test_fov=test_fov,
            loader=pil_loader, transform=transform,target_transform = target_transform)
    shuffle = True if train is True else False 
    data_loader = torch.utils.data.DataLoader(dataset, batch_size=batch_size, 
