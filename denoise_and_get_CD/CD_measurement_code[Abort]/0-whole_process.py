...
parser.add_argument('--target', default='F16_v2', type=str,
                choices = ['F16_v2','F16_v1','F08_v2','median'])
...
with open(f'./tmp/result_data/hole_info_{target}.txt', 'r') as f:
    hole_info = json.load(f)



wandb.init(
    project="CD_process_median",
    config={
        "target": target,
        "weight": fbi_weight_dir,
        "pad": pad,
    }
)
...
img_dict = np.load("./tmp/intermediate_result/full_img_dict.npy",allow_pickle=True).item()
...
if target == 'F16_v2' or os.path.isfile("./tmp/result_data/CD_dict_F16_v2&F16_v2&median_median.csv") is False:
    method_list.append("median")
if "median_input" in fbi_weight_dir:
    method_list = [f"median_{target}"]
...
for img_info, img in return_img(img_dict):
    ...
    for method in method_list:
        ...
        img_info['fbi_weight_dir'] = fbi_weight_dir.split("/")[-1]
        target_model = model
        print("method :",method)
        if f"{target}_median" == method:
            target_model = denoise_and_median
            print("denoise_and_median")
        elif f"median_{target}" == method:
            target_model = median_and_denoise
            print("median_and_denoise")
        elif method == 'median':
            print("median")
        ...
        CD_info_ours = CD_process(target_model, img, img_info, img_hole_info, pad, save_img=True)
        CD_dict = get_metric(CD_dict, CD_info_ours,key, collect_metric_list, method)
       
    print("")
CD_dict = pd.DataFrame(CD_dict)
CD_dict.to_csv(f"./tmp/result_data/CD_dict_{method_str}_{save_weight_name}.csv")

wandb.log({"CD_dict": wandb.Table(dataframe=CD_dict)})
