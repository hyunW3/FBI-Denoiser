    ...
    if args.noise_type == 'Poisson-Gaussian':
        ...
        if args.data_type == 'RawRGB' and args.data_name == 'fivek' and args.alpha == 0 and args.beta == 0:
            if args.test_alpha != 0 or args.test_beta != 0:
                te_data_dir = './test_fivek_rawRGB_alpha_'+str(args.test_alpha)+'_beta_'+str(args.test_beta)+f'_{num_sampling}sampling{num_images}images.hdf5'
    
                save_file_name = f"{args.date}_{args.model_type}_{args.data_type}_trainon_random_noise_teston_alpha_{args.test_alpha}_beta_{args.test_beta}_{num_sampling}sampling{num_images}images"
            else :
                print("random noise dataset not exist")
                sys.exit(-1)
                te_data_dir = './data/test_fivek_rawRGB_random_noise.hdf5'
                save_file_name = str(args.date)+ '_'+str(args.model_type)+'_' + str(args.data_type) +'_' + f'random_noise_{num_sampling}sampling{num_images}images'
            ...
            if args.test_alpha != 0 or args.test_beta != 0:
                te_data_dir = './test_fivek_rawRGB_alpha_'+str(args.test_alpha)+'_beta_'+str(args.test_beta)+f'_{num_sampling}sampling{num_images}images.hdf5'
                save_file_name = f"{args.date}_{args.model_type}_{args.data_type}_trainon_alpha_{args.alpha}_beta_{args.beta}_teston_alpha_{args.test_alpha}_beta_{args.test_beta}_{num_sampling}sampling{num_images}images"
            else :
                te_data_dir = './test_fivek_rawRGB_alpha_'+str(args.alpha)+'_beta_'+str(args.beta)+f'_{num_sampling}sampling{num_images}images.hdf5'
                save_file_name = f"{args.date}_{args.model_type}_{args.data_type}_trainon_alpha_{args.alpha}_beta_{args.beta}_{num_sampling}sampling{num_images}images"
            ...
            print("Samsung dataset is now off")
            sys.exit(-1)
            """
